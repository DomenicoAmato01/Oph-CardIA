...
def apply_fcm(image: np.ndarray, n_clusters: int = 2) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
	# Reshape image to (num_pixels, 1)
	pixels = image.reshape(-1, 1).astype(np.float32)

	# Apply FCM with initial centers set to 8 and 250
	# create FCM and try to seed initial centers if possible
	fcm = FCM(n_clusters=n_clusters, m=2.0, max_iter=100, error=1e-5, n_jobs=1)
	# attempt to set initial centers -- fcmeans may accept setting attribute
	initial_centers = np.array([[8.0], [250.0]], dtype=np.float64)
	try:
		fcm.centers = initial_centers
	except Exception:
		# some implementations ignore this; we'll continue and rely on fit
		pass
	...
	centers = np.asarray(fcm.centers).flatten()
	...
	for i, center in enumerate(centers):
		# clamp center to 0-255 and cast
		val = int(np.clip(center, 0, 255))
		segmented[labels == i] = val

	# Build IMMM: map vessel cluster to high value and background to low value
	# determine vessel cluster as one with larger center (similar to MATLAB script)
	if centers.size >= 2:
		vessel_label = int(np.argmax(centers))
	else:
		vessel_label = 0

	IMMM = np.where(labels == vessel_label, 254.0, 8.0)

	# Estimate background with large structural element and subtract
